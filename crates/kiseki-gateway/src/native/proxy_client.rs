//! Server-side proxy fallback client (ADR-044 native row + ADR-042 §4).
//!
...
/// hop budget is auditable from one place. Value chosen per ADR-044
...
///     (ADR-044 §"Consequences"), so contention is low;
