...
//!
//! Phase 14d adds an `ObjectBackupBackend` trait so snapshots can land
//! on local disk OR an S3-compatible store (production target per
//! ADR-016 §"External backup"). The default implementation
//! [`FileSystemBackupBackend`] preserves the previous on-disk
//! behaviour; [`S3BackupBackend`] (separate module) talks to a real
//! object store.
//!
