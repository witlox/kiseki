...
fn issue_handle(world: &mut KisekiWorld, expiry_ms: u64, stripe: u32) -> PnfsFileHandle {
    ...
    let ns = world.legacy.nfs_ctx.namespace_id;
    let comp = world
        .last_composition_id
...
fn run_compound(
    ...
    for _ in 0..num_ops {
        ...
        // is per-scenario.
        world.pnfs.last_results.push((op_code, status, Vec::new()));
    }
}
...
async fn given_pnfs_cluster(world: &mut KisekiWorld) {
    ...
    world.pnfs.gateway_reads = Arc::new(std::sync::atomic::AtomicU64::new(0));
    world.pnfs.last_results.clear();
}
...
async fn given_composition_with_size(
    ...
    world.pnfs.composition_bytes = Some(bytes.clone());
...
async fn when_client_reads_via_ds(world: &mut KisekiWorld, _stripe: u32, offset: u64, length: u32) {
    ...
    let fh = world.pnfs.fh.clone().expect("fh4 issued");
...
async fn then_ds_returns_ok_with_bytes(world: &mut KisekiWorld, _n: u32) {
    ...
    let read = world.pnfs.last_results.get(1).expect("READ result");
...
async fn given_forged_fh4(world: &mut KisekiWorld) {
    ...
    let comp = world
        .last_composition_id
        .unwrap_or_else(|| CompositionId(uuid::Uuid::from_u128(0xC0_FFEE)));
    world.pnfs.fh = Some(PnfsFileHandle::issue(
        &other_key,
        tenant,
        ns,
        comp,
...
async fn then_notsupp(world: &mut KisekiWorld) {
    let putfh = world.pnfs.last_results.first().expect("PUTFH result");
    assert_eq!(putfh.1, kiseki_gateway::nfs4_server::nfs4_status::NFS4_OK);
    let alloc = world.pnfs.last_results.get(1).expect("ALLOCATE result");
    assert_eq!(
        alloc.1,
...
async fn given_only_env(world: &mut KisekiWorld) {
    ...
    // pollute other tests).
    world.pnfs.security_eval = Some(evaluate(false, true, true, 300, 1));
}
...
async fn given_only_config(world: &mut KisekiWorld) {
    world.pnfs.security_eval = Some(evaluate(true, false, true, 300, 1));
}
...
async fn given_both_flags(world: &mut KisekiWorld) {
    ...
    world.pnfs.security_eval = Some(evaluate(true, true, false, 300, 1));
}
...
async fn then_audit_emitted(world: &mut KisekiWorld) {
    ...
    let ts = now_ms();
    world.pnfs.audit_log.append(AuditEvent {
    ...
    assert!(!world.pnfs.audit_log.query(&q).is_empty());
}
...
async fn then_warn_banner(world: &mut KisekiWorld) {
    let res = world.pnfs.security_eval.as_ref().expect("evaluated");
    let s = res.as_ref().expect("expected Ok");
...
async fn given_two_tenants(world: &mut KisekiWorld) {
    ...
    world.pnfs.security_eval = Some(evaluate(true, true, false, 300, 2));
}
...
fn ensure_mds_mgr(world: &mut KisekiWorld) -> Arc<MdsLayoutManager> {
    if let Some(ref m) = world.pnfs.mds_mgr {
        return Arc::clone(m);
    }
    let key = world
        .pnfs
    ...
    world.pnfs.mds_mgr = Some(Arc::clone(&mgr));
    mgr
}
...
async fn when_layoutget(world: &mut KisekiWorld, name: String, start: u64, end_mib: u32) {
    ...
    let now = world.pnfs.clock_ms;
    let layout = mgr.layout_get(
        world.legacy.nfs_ctx.tenant_id,
        world.legacy.nfs_ctx.namespace_id,
        comp,
        start,
        length,
        LayoutIoMode::Read,
...
async fn then_fh4_size(world: &mut KisekiWorld, total: u32, _payload: u32, _mac: u32) {
    ...
    for m in &layout.stripes {
        ...
        m.fh.validate(&key, world.pnfs.clock_ms)
            .expect("fh validates");
    }
}
...
async fn then_ff_device_addr(world: &mut KisekiWorld) {
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    let layout = world.pnfs.last_layout.as_ref().expect("LAYOUTGET ran");
...
async fn given_layout_ttl_ms(world: &mut KisekiWorld, ms: u64) {
    ...
    world.pnfs.mds_mgr = Some(Arc::new(MdsLayoutManager::new(key, cfg)));
}
...
async fn when_ms_elapse(world: &mut KisekiWorld, ms: u64) {
    world.pnfs.clock_ms = world.pnfs.clock_ms.saturating_add(ms);
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    let _evicted = mgr.sweep_expired(world.pnfs.clock_ms);
}
...
async fn when_n_layoutgets_distinct(world: &mut KisekiWorld, n: u32) {
    let mgr = ensure_mds_mgr(world);
    let mut now = world.pnfs.clock_ms;
    for i in 0..n {
...
fn ensure_bus(world: &mut KisekiWorld) -> Arc<TopologyEventBus> {
    if let Some(ref b) = world.legacy.topology_bus {
        return Arc::clone(b);
    }
    let bus = Arc::new(TopologyEventBus::new());
    world.legacy.topology_bus = Some(Arc::clone(&bus));
    bus
}
...
async fn given_topology_subscriber(world: &mut KisekiWorld) {
    ...
    world.legacy.topology_sub = Some(bus.subscribe());
}
...
async fn when_drain_commits(world: &mut KisekiWorld, name: String) {
    ...
    world.raft.node_names.insert(name, target);
...
async fn then_event_after_commit(world: &mut KisekiWorld) {
    ...
    let bus = world.legacy.topology_bus.as_ref().expect("bus");
...
async fn then_one_split_event(world: &mut KisekiWorld) {
    let sub = world.legacy.topology_sub.as_mut().expect("subscriber");
    match sub.try_recv() {
...
async fn given_slow_subscriber(world: &mut KisekiWorld) {
    ...
    world.legacy.topology_sub = Some(bus.subscribe());
    world.legacy.topology_bus = Some(bus);
}
...
async fn when_drain_commits_named(world: &mut KisekiWorld, _name: String) {
    ...
    let event_hlc = world.pnfs.clock_ms;
    world.pnfs.clock_ms = world.pnfs.clock_ms.saturating_add(50); // ~50 ms recall latency
...
async fn then_recalled_fh4_badhandle(world: &mut KisekiWorld) {
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    let layout = world.pnfs.last_layout.as_ref().expect("layout was issued");
    let fh = layout
        .stripes
        .iter()
...
async fn when_split_commits(world: &mut KisekiWorld) {
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    let event_hlc = world.pnfs.clock_ms;
    world.pnfs.clock_ms = world.pnfs.clock_ms.saturating_add(20);
    // ShardSplit is conservative (recall_all) per Phase 15c §D6.
    let _ = mgr.recall_all(RecallReason::ShardSplit, event_hlc, world.pnfs.clock_ms);
}
...
async fn given_n_layouts_m_comps(world: &mut KisekiWorld, n: u32, _m: u32) {
    ...
    world.pnfs.clock_ms = world.pnfs.clock_ms.saturating_add(10);
}
...
async fn when_k_layout_rotated(world: &mut KisekiWorld) {
    ...
    mgr.rotate_mac_key(new_key.clone(), event_hlc, world.pnfs.clock_ms);
    world.pnfs.mac_key = Some(new_key);
}
...
async fn then_new_layouts_validate_new_key(world: &mut KisekiWorld) {
    ...
    layout.stripes[0]
        .fh
        .validate(&key, world.pnfs.clock_ms)
...
async fn when_seconds_elapse(world: &mut KisekiWorld, s: u64) {
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    world.pnfs.clock_ms = world.pnfs.clock_ms.saturating_add(s * 1_000);
    let _ = mgr.sweep_expired(world.pnfs.clock_ms);
}
...
async fn then_subsequent_badhandle(world: &mut KisekiWorld) {
    let mgr = world.pnfs.mds_mgr.clone().expect("manager wired");
    let layout = world.pnfs.last_layout.as_ref().expect("layout issued");
    ...
    // world.pnfs.clock_ms via the build_ds_ctx fixed-clock.
    assert_eq!(
        putfh.1,
        kiseki_gateway::nfs4_server::nfs4_status::NFS4ERR_BADHANDLE
    );
...
async fn when_lag_observed(world: &mut KisekiWorld) {
    ...
    world.legacy.topology_bus = Some(bus);
    world.legacy.topology_sub = Some(sub);
