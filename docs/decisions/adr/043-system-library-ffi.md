...
| `libfuse` 3.x | FUSE protocol dispatch | `kiseki-fuse-sys` (bindgen) + `kiseki-fuse` safe wrapper | No | userspace ≥ 3.10 + kernel ≥ 5.4 | **New rev-2 addition.** Replaces `fuser` 0.17. The userspace floor (libfuse 3.10) is the Debian/Ubuntu LTS shipped version. The kernel floor (≥ 5.4) covers FUSE_SYNCFS opcode 50 dispatch (kernel-side; the kernel sends the opcode on ≥ 5.1 — kiseki requires ≥ 5.4 for stability). **FUSE_SYNCFS userspace handling deferred to libfuse 3.19+** (Option A of `specs/escalations/2026-05-09-libfuse-syncfs-not-in-318-release.md`, accepted 2026-05-09): libfuse 3.18.2 (latest tagged) has no `fuse_lowlevel_ops::syncfs` callback — it landed on master commit `074f0dfc` (2026-01-29) but is not yet in any release. Until libfuse 3.19 ships, kiseki replies `ENOSYS` to the kernel for FUSE_SYNCFS; the kernel falls back to per-inode `FUSE_FSYNC` which routes through our existing `fsync` hook. End-user `sync(2)` semantics preserved. Operator docs document the kernel floor at acceptance of the libfuse swap. Implementation plan: `specs/implementation/libfuse-swap.md`. No per-binding ADR per §D6. **D1.1 data**: upstream `libfuse/libfuse`; security advisories tracked at the upstream's GitHub Security Advisories; CVE history filled in by libfuse-swap.md acceptance criterion (7); kiseki triage SLA = D1.1 default. |
...
- `kiseki-client`'s FUSE adapter drops the `fuser` 0.17 dependency and its known gaps (single-thread inline dispatch; smaller maintainer pool than libfuse). Inherits libfuse 3.x's multi-thread session loop and the bug-history depth of the reference impl. The FUSE_SYNCFS userspace handling deferred to libfuse 3.19+ per the §D2 row above; the kernel ENOSYS-fallback path is preserved correctness-wise via `FUSE_FSYNC` per open inode.
