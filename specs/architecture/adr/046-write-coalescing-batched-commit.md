...
**Status**: Proposed — adversary gate-1 complete 2026-05-28 (1C + 3H + 3M + 2L; see `specs/findings/2026-05-28-adv-gate1-adr046-write-coalescing.md`), all resolved in **rev-2** below. **Release R landed** 2026-05-28 on `perf/2026-05-28-roadmap` (decode + atomic apply with per-item HLC `logical` + `append_batch_chunk_and_delta` API + the per-shard `WriteCoalescer` queue) — all built + unit-tested, **hard-gated off / not wired into the write path**. **R+1 pending**: wire the queue into the gateway write path behind the committed `WriteCoalesceEnabled` capability (C1), the per-waiter `ForwardToLeader` re-route (M3), and the `batch_size` metric (L2).
