...
**Status**: Accepted — adversary gate-1 complete 2026-05-28 (1C + 3H + 3M + 2L; see `specs/findings/2026-05-28-adv-gate1-adr046-write-coalescing.md`), all resolved in **rev-2** below. **Release R landed** 2026-05-28 on `perf/2026-05-28-roadmap` (decode + atomic apply with per-item HLC `logical` + `append_batch_chunk_and_delta` API + the per-shard `WriteCoalescer` queue) — built + unit-tested, hard-gated off.

**Release R+1 landed** 2026-05-28 (same branch) — **emission wired**:
- The `WriteCoalescer` queue moved from `kiseki-gateway` to `kiseki-log`. Forwarded writes (#114) reach the leader via `LogService.AppendChunkAndDelta` and converge with local writes at `RaftShardStore::append_chunk_and_delta[_with_forwarding]` — coalescing must happen there to catch BOTH paths (gateway-side would only catch ~1/N of writes under distributed shard leadership).
- `RaftShardStore` holds an optional per-shard coalescer (over a `ShardMapAppender` that shares the live shard map — no `Arc` cycle). Both append methods route through it when enabled; the legacy `append_chunk_and_delta` collapses `ForwardToLeader → LeaderUnavailable` per its contract, while `_with_forwarding` preserves the hint. The queue's error fan-out reconstructs `ForwardToLeader` (M3) so a follower's coalesced write still re-routes through the gateway/native proxy.
- Integration test (`coalesced_concurrent_appends_all_commit_with_distinct_seqs`): 16 concurrent appends coalesce into fewer Raft entries yet each commits with a distinct seq and every delta replays — no lost/aliased writes.

**Gate (rev-2 C1) — what landed vs deferred (be honest):** R+1 ships the **operator permit** `KISEKI_WRITE_COALESCE` (default **OFF**), NOT yet the *automatic committed cluster-wide `WriteCoalesceEnabled` capability* described in §C1 below. The env permit is sufficient + safe for the **current pre-production reality** (fresh same-version clusters; the GCP perf run deploys one binary to all 6 nodes — no rolling upgrade), where the operator enables it only after confirming every node decodes `BatchChunkAndDelta`. **The committed-capability negotiation (the automatic fail-safe authorizer) MUST land before the first production rolling upgrade across mixed binary versions** — until then, enabling the permit mid-rollout from a pre-R binary is unsafe. Tracked as the C1 production-hardening follow-up. **L2** `batch_size` is trace-level (`write-coalesce: flushed batch`), not a registered histogram — the throughput lift is measured via the existing `raft_commit` put-phase histogram.
