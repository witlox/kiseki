...
| I-L4 (GC requires all consumers advanced) | `kiseki-log` watermark-advance GC | ConsumerWatermarks checked; min(all watermarks) = GC boundary (deltas with `sequence < boundary` pruned; `== boundary` survives). Enforced at four points: `raft/state_machine.rs` prunes on every applied `AdvanceWatermark` (P3a — replicated, deterministic on all replicas); `raft_shard_store.rs` supervisor watermark round proposes min over EVERY voter's reported hydrator position (any missing voter skips the round); membership-add guard (`OpenRaftLogStore::guard_replica_add` + the `test_cluster.rs` add paths) refuses a NEW replica with `DeltaLogPruned` once boundary > 1 (a fresh replica's hydrator cannot replay pruned history — ADR-040 §D6.3); admin gRPC boundary (`kiseki-server/storage_admin.rs` SplitShard/MergeShards) fail-fast-refuses full-replay lifecycle ops with FAILED_PRECONDITION BEFORE the control-plane RecordSplit/RecordMerge mutates topology (no dangling shard group on refusal; the deep gates in `redistribute_upper_half` / `merge::copy_phase` remain as defense in depth). BDD: multi-node-raft.feature "SplitShard on a pruned delta log is refused" via the `/admin/test/shard/{id}/advance-watermark/{pos}` knob. |
...
| I-C2 (no GC while refcount > 0) | `kiseki-log` Raft state machine + `cluster_chunk_state` apply path (Phase 16a); follower-ingress decrement forward in `mem_gateway` delete + `AppendForwarder::forward_decrement_chunk_refcount` → `LogService.DecrementChunkRefcount` (#111 delete half) | Refcount lives in the per-shard `cluster_chunk_state[(tenant_id, chunk_id)]` Raft table; ChunkAndDelta / IncrementChunkRefcount / DecrementChunkRefcount transitions are atomic with the composition delta they reference. The decrement is a Raft `client_write` (leader-only): a DELETE served by a non-leader node forwards it to the shard leader instead of dropping it (the pre-2026-06-11 silent-swallow leaked refcounts forever). Local `kiseki-chunk` GC sweep only fires once cluster_chunk_state.refcount hits 0 and the leader has fanned out DeleteFragment to every peer in the placement list. GH #241: a decrement applying BEFORE its chunk's create incorporation (ADR-047 async-ack window) parks in the SM's `pending_chunk_decrements` and drains inside `apply_new_chunks` when the row materializes — refcount 0 + tombstoned at birth, deterministic on every replica (the pre-fix no-op leaked the refcount permanently). |
...
| Invariant | Enforcement point | Mechanism |
|---|---|---|
| I-PN1 (fh4 MAC + expiry) | `kiseki-gateway` DS request handler | On every DS op: recompute HMAC-SHA256 over `(tenant‖ns‖comp‖stripe‖expiry_ms)` with key `HKDF(master, info=b"kiseki/pnfs-fh/v1")`; constant-time compare against `fh.mac`; check `expiry_ms > now_ms`; failure → `NFS4ERR_BADHANDLE`. |
| I-PN2 (DS stateless) | `kiseki-gateway` DS handler design | DS module owns no `RwLock<HashMap>` for opens, locks, layouts, or stateids. Crash recovery is restart-and-resume; tested by killing the DS task mid-flight and asserting client succeeds via fresh-fh4 retry. Audit by structural lint: no `static` mutable state in `pnfs_ds_server.rs`. |
| I-PN3 (DS plaintext boundary) | `kiseki-gateway` DS → `GatewayOps` | DS handler decodes fh4 → builds `ReadRequest`/`WriteRequest` against `kiseki-gateway::ops::GatewayOps`. The trait already encrypts on write / decrypts on read via `kiseki-composition` → `kiseki-chunk` → `kiseki-crypto`. No DS-side crypto code. |
| I-PN4 (layout TTL ≤ 5 min) | `kiseki-gateway` `LayoutManagerOps::layout_get` | `expiry_ms = now_ms + layout_ttl_seconds * 1000` stamped into every fh4 in the layout. DS rejects expired fh4s (I-PN1). Layout cache evicts entries older than TTL on read. |
| I-PN5 (recall fires on topology event) | `kiseki-control` event hooks → `kiseki-gateway` | ADR-035 drain transition emits `NodeStateChanged{Drain}` → MDS subscribes via existing event channel and calls `LayoutManagerOps::layout_recall` for each affected composition. ADR-033/034 split/merge events similarly. fh4 MAC key rotation → bulk recall. SLA: 1 sec from event commit to recall send-out. |
| I-PN6 (layouts respect namespace shard map) | `kiseki-gateway` `LayoutManagerOps::layout_get` | Stripe membership = active nodes from `GetNamespaceShardMap(ns)` (ADR-033). Drained nodes (ADR-035 state machine) are filtered out before stripe assignment. Property test: random topology + drains → no layout includes a drained node. |
| I-PN7 (TLS default + audited fallback + DS op subset) | `kiseki-gateway` MDS+DS listener config; `kiseki-server` config-load + audit emitter | Both listeners wrap `TcpListener` with `TlsConfig::server_config` (Cluster CA per ADR-009) using the same cert/key/CA bundle already loaded for the gRPC data path. Plaintext bind allowed only if **both** `[security].allow_plaintext_nfs=true` AND `KISEKI_INSECURE_NFS=true` (process-start env). On plaintext: emit `SecurityDowngradeEnabled` audit event each boot, log a STARTUP banner, halve `layout_ttl_seconds` to 60s, refuse start if `tenant_count > 1` for any served namespace. Op dispatcher is a fixed match arm allowing only `EXCHANGE_ID`, `CREATE_SESSION`, `DESTROY_SESSION`, `PUTFH`, `READ`, `WRITE`, `COMMIT`, `GETATTR`; default → `NFS4ERR_NOTSUPP`. |
| I-PN8 (layout cache bounded) | `kiseki-gateway` `LayoutManagerOps` impl | HashMap with explicit `max_entries` cap (default 100k). Background sweeper task runs every `sweep_interval_seconds` (default 75s) removing entries with `now_ms > issued_at_ms + ttl_ms`. On capacity, evict smallest `issued_at_ms` entry on insert. Sweeper is single-threaded, no MDS RPC. Property test: 1M random LAYOUTGETs over 1 hour → live-set bounded by cap × 1.1 (10% slack for in-flight inserts). |
| I-PN9 (topology event bus delivery) | `kiseki-control` `TopologyEventBus` + `kiseki-gateway` subscriber | Producers (drain orchestrator, namespace shard map mutator, composition delete handler, key rotation handler) call `bus.emit(event)` **after** their respective control-Raft commit. Single `tokio::sync::broadcast::Sender<TopologyEvent>`, capacity 1024. Gateway subscribes at startup, processes events in a dedicated task. Lag → full layout-cache flush + `pnfs_topology_event_lag_total` counter increment. I-PN4 TTL remains ultimate fallback (test: kill the subscriber task, drain a node, verify recall fires within ≤ TTL). |
