Feature: Native Gateway Data Service — gRPC data-plane for native clients
  ...
  # ADR-044 server-side leader-forwarding posture. The Step A
  ...
  # ADR-044 gate-1 defenses — these CAN be exercised end-to-end
  ...
  @native @routing
  Scenario: ADR-044 proxy gate refuses self-forward (gate-1 C-H2)
  ...
  @native @routing
  Scenario: ADR-044 proxy gate enforces hop cap (gate-1 C-H4)
  ...
  @native @routing
  Scenario: ADR-044 proxy gate rejects unknown leader
