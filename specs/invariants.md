...
| I-L2 | A write is acked once its chunks (I-L5) **and** its delta are durably recorded on a quorum (`min_acks`) of replicas, with an ingress-assigned **perspective sequence** — the universal no-loss floor. The single total order (I-L1) and reader visibility come from per-shard Raft applied to that quorum-durable, perspective-sequenced intent, and this **MAY complete asynchronously to the ack** (never split-brain — one recoverable order; reader visibility bounded-stale per I-CS2). EC/replication provides *data* durability; Raft provides *ordering* — the two are decoupled, so the ack waits on the quorum floor (data + metadata durable), **not** on the Raft consensus round. No acked write is ever lost. (Impl note: current impl is the stricter synchronous form — Raft commit+apply on the ack path; relaxing it to this floor is the open write-perf work.) | Proposed (rev 2026-05-29 — ADR-047, gate-1 passed; pending impl + gate-2) |
...
| ID | Invariant | Status |
|---|---|---|
| I-CS1 | No data loss, **universal** (NOT scoped to regulated data): a write is not acknowledged until its data **and** metadata are durable on a quorum (`min_acks`) of replicas. A single global order with no split-brain is provided by per-shard Raft over the perspective-sequenced quorum intent, applied **asynchronously to the ack**. Synchronous Raft-commit-before-ack is not required for safety — quorum durability is the floor; ordering is async-but-guaranteed (recoverable from the durable intent). **Visibility is per protocol surface (ADR-047 §F-3): S3 / object / native = async (bounded-stale, I-CS2); POSIX / NFS / FUSE = synchronous-apply (close-to-open, ADR-013).** | Proposed (rev 2026-05-29 — ADR-047, gate-1 passed; pending impl + gate-2) |
| I-CS2 | Bounded staleness for reads: acceptable per view descriptor, subject to compliance floor (I-K9). | Confirmed |
| I-CS3 | Federated sites are eventually consistent (async replication). No cross-site Raft. Per-site: the I-CS1 quorum-durable no-loss floor (ordering applied async per ADR-047; visibility per-surface). | Confirmed |
...
| I-NG2 | Object-flavored writes are atomic *commit-on-close*. A stream interrupted before `CommitStream` is server-side garbage-collected within the idempotency-key dedup window (default 5 min) and partial state is **never** visible. `CommitStream` MUST NOT return `Ok` until every chunk **and** the composition delta are quorum-durable on `min_acks` peers per the pool's durability strategy (the I-L2/I-L5 no-loss floor). **Visibility after `Ok` is per protocol surface (ADR-047 §F-3): synchronous-apply surfaces (POSIX/NFS/FUSE, ADR-013) are visible at `Ok`; async surfaces (S3/object/native, ADR-014) — `Ok` guarantees quorum-durability (no loss), and visibility follows the asynchronous apply within I-CS2.** All-or-nothing atomicity (no partial composition ever visible) holds on both. | Proposed |
