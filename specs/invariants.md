...
| I-CP5 | When the openraft log compaction window advances past `meta.last_applied_seq`, the hydrator detects the gap by sequence-comparison on `read_deltas` results (per ADR-040 §D6.3) and enters halt mode: emits one throttled `tracing::error!` per minute, sets `kiseki_composition_hydrator_stalled = 1`, stops polling. Existing reads continue to be served from the persistent store. The operator's recovery action is to drop the node's metadata redbs and restart; the persistent store re-hydrates from the next openraft snapshot. (Until the deferred snapshot-bundle ADR lands, this is the only correct behavior.) | Proposed |
...
| ID | Invariant | Status |
|---|---|---|
| I-FUSE-1 | Every libfuse reply token (`ReplyAttr`, `ReplyData`, `ReplyEmpty`, `ReplyWrite`, `ReplyCreate`, `ReplyEntry`, `ReplyOpen`, `ReplyDirectory`, `ReplyStatfs`, `ReplyXattr`, `ReplyLock`) is consume-once: it MUST be passed to exactly one `.attr(...)` / `.data(...)` / `.ok()` / etc. call before being dropped. After the consume call, the token's `Drop` is a no-op. | Spec-only (kiseki-fuse pending) |
| I-FUSE-2 | If a Rust handler returns or panics without consuming its reply token, the token's `Drop` impl issues `fuse_reply_err(req, EIO)` to libfuse, increments the Prometheus counter `kiseki_fuse_drop_without_consume_total{op}`, and logs at WARN level. Debug builds additionally panic. Guarantees libfuse never holds a request slot indefinitely. | Spec-only (kiseki-fuse pending) |
| I-FUSE-3 | Reply tokens cross the libfuse-session-thread / tokio-task boundary ONLY through a wrapper-provided `tokio::sync::oneshot::{Sender, Receiver}<Result<ReplyResult, FuseError>>` bridge. The session thread blocks on `recv()` with a per-request bounded timeout (default 30 s, configurable). Reply tokens are `!Send` outside the bridge; sending one via direct `tokio::spawn` fails to compile. | Spec-only (kiseki-fuse pending) |
| I-FUSE-4 | Cancellation of an async handler future MUST produce `EINTR` to the kernel (not a leaked slot or default-EIO). Plaintext buffers handled by the cancelled future MUST be zeroized on drop via the `ZeroOnCancel<Vec<u8>>` smart pointer the wrapper provides; this extends the existing `Zeroizing<Vec<u8>>` discipline used in `mem_gateway::DecryptCache` to the cancellation path. | Spec-only (kiseki-fuse pending) |
| I-FUSE-5 | The libfuse session loop MUST run on a dedicated `std::thread` named `kiseki-fuse-session`, NOT on `tokio::task::spawn_blocking`. The dedicated thread isolates session work from kiseki's other blocking work (which competes for tokio's blocking-pool slots, default cap 512). | Spec-only (kiseki-fuse pending) |
| I-FUSE-6 | The wrapper's `Filesystem` trait is `Send + Sync + 'static`. Reply-token lifetime is the tightly-constrained surface (per I-FUSE-1..3); the handler's own state is unconstrained beyond `Send + Sync`. | Spec-only (kiseki-fuse pending) |
| I-FUSE-7 | The wrapper enforces `max_pending_ops` (default 1024) on the in-flight bridge map. When the cap is hit, new FUSE requests are immediately replied with `EAGAIN` rather than queued. Configurable via `KisekiFuseConfig::max_pending_ops`. | Spec-only (kiseki-fuse pending) |
| I-FUSE-8 | If the libfuse session thread exits unexpectedly (panic, EIO from kernel `/dev/fuse`, signal), the wrapper detects this via the thread's join handle. Default behavior: abort the kiseki-client process (fail-fast, preserves the pre-swap process-crash shape). Operator-opt-in via `KisekiFuseConfig::auto_remount = true`: attempts a single re-mount with the same options; on second crash falls back to abort. | Spec-only (kiseki-fuse pending) |
