...
| **Advisory audit event** | Audit log entry for a Workflow Advisory decision: `declare`, `end`, `phase-advance`, `hint-accepted`, `hint-rejected`, `hint-throttled`, `telemetry-subscribed`, `budget-exceeded`. Routed to the tenant audit shard. | Workflow Advisory / Audit | Same durability and scoping guarantees as all other tenant audit events (ADR-009). |
