# Adversary gate-1 — ADR-042 §4 + Step A architect output

**Date**: 2026-05-15
**Reviewer**: Adversary (same agent advancing through the role chain)
**Targets**: `specs/findings/2026-05-15-leader-forwarding-posture.md` (analyst output), commits `65ef681` (ADR-042 §4) + `9ab4920` (architect — LogError variant + `append_delta_with_forwarding`).
...
**Spec reference**: ADR-042 §4 §"Decision" — native row; ADR-042 §4.
...
**Resolution**: **acceptable for Step A**. `kiseki-log::grpc` is the Raft cross-node log service used by replication, NOT by gateway clients. ADR-042 §4 native proxy operates at a different transport (gateway data port, not Raft port). The native gateway calls `OpenRaftLogStore` *directly* through `kiseki-gateway::mem_gateway` — never over `kiseki-log::grpc`. So this mapping is dead-code-equivalent for the ADR-042 §4 path. No action required for Step A. **Re-open** if a future client design wants to use kiseki-log::grpc as a write proxy (which would conflict with ADR-026 anyway).
...
**Spec reference**: ADR-042 §4 §"Hop cap" + ADR-042 §4 "trailing metadata".
...
**Spec reference**: ADR-042 §6 + ADR-042 §4 §"Adversary gate 1 — H2".
...
**Risk**: operators can't distinguish "leader stable but follower received write" (`ForwardToLeader`) from "election in progress" (`LeaderUnavailable`) from generic Raft failure (`Unavailable`) in the metrics. The cluster-health alarm in ADR-042 §4 §"Consequences" (`kiseki_native_proxy_forwards_total{source_node, leader_node}` > 20% sustained) needs this distinction.
...
1. **Look up `data_addr` from the local topology cache** (`kiseki-control::cluster_control::state_machine`). This is what ADR-042 §4 §"Implementation map" specifies. Works because the control plane keeps per-node addresses globally synced. Adversary preference: this option, because it keeps `LogError` payload minimal and aligns with ADR-042 §4 "topology cache" architecture.
...
**Spec reference**: ADR-042 §4 §"Implementation map" + ADR-026 (Raft topology) + ADR-008 (discovery).
...
**Spec reference**: ADR-042 §4 §"Hop cap" + this doc's C-H2.
...
- 4 MEDIUM findings — **all resolved or accepted**:
  ...
  - C-M4: implementer uses topology-cache lookup (architectural decision recorded in ADR-042 §4 §"Implementation map").
...
5. ADR-042 §4 §"Adversary gate 1 — H1" (no early-ack short-circuit on proxy) — code inspection.
6. ADR-042 §4 §"Adversary gate 1 — H2" (proxy-node-death + idempotency replay) — BDD scenario at `specs/features/native-gateway.feature:172-177` is GREEN.
